...
while(!q.empty()){
    if(q.front().first==q.front().second)v.push_back(q.front().first);
    else{
            int c=(q.front().first+q.front().second)/2;
        v.push_back(c);
        q.push(std::make_pair(q.front().first,c-1));
        q.push(std::make_pair(c+1,q.front().second));
    }
    q.pop();
}
for(int x:v)bst[x]=x;
